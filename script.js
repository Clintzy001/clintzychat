...
// --- 1. CORE UTILITIES (CAMERA & CALLS) ---

// Real Camera Functionality
async function openCamera() {
    try {
        const stream = await navigator.mediaDevices.getUserMedia({ video: true });
        alert("Camera accessed! In a full app, we would show the viewfinder here.");
        stream.getTracks().forEach(track => track.stop()); // Stop camera after test
    } catch (err) {
        alert("Camera Error: Please ensure you are on HTTPS or have given permission.");
    }
}

function startCall(type, name) {
    const icon = type === 'video' ? '📹' : '📞';
    alert(`${icon} Starting ${type} call with ${name}...`);
}

// --- 2. RENDER CHATS WITH FILTERING ---
...
function handleMenuAction(action) {
    ...
    switch(action) {
        case 'new-group': alert('Create New Group initialized'); break;
        case 'read-all': 
            chats.forEach(c => c.unread = 0); 
            renderChats(); 
            break;
        case 'settings': navigate('settings'); break;
        case 'linked-devices': alert('Scanning for Linked Devices...'); break;
        default: alert('Action: ' + action);
    }
}
...
function sendMessage() {
    ...
    msgDiv.innerHTML = `<div class="bubble-orange p-3 max-w-[80%] text-sm font-medium">${input.value}</div>`;
...
function openConversation(id) {
    ...
    windowOverlay.innerHTML = `
        <header class="p-4 flex items-center bg-black/95 border-b border-zinc-900">
            <button onclick="closeChat()" class="text-orange-500 mr-2"><i data-lucide="chevron-left"></i></button>
            ...
            <div class="flex space-x-6 text-zinc-400">
                <button onclick="startCall('voice', '${chat.name}')"><i data-lucide="phone" class="w-5 h-5"></i></button>
                <button onclick="startCall('video', '${chat.name}')"><i data-lucide="video" class="w-5 h-5"></i></button>
            </div>
        </header>
        ...
        <footer class="p-3 bg-black border-t border-zinc-900 flex items-center space-x-2">
            ...
            <button onclick="sendMessage()" class="bg-orange-500 p-2 rounded-full text-black">
...
function closeChat() { document.getElementById('chat-window').classList.add('translate-x-full'); }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Bind global camera icon
    const camBtn = document.querySelector('[data-lucide="camera"]');
    if (camBtn) camBtn.parentElement.onclick = openCamera;
